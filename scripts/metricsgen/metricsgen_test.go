...
import (
	...
	metricsgen "github.com/ice-blockchain/cometbft/scripts/metricsgen" //nolint:revive // this only works with metricsgen in front
)
...
func TestParseAliasedMetric(t *testing.T) {
	aliasedData := `
			...
			import(
				mymetrics "github.com/ice-blockchain/cometbft/libs/metrics"
			)
			type Metrics struct {
				m mymetrics.Gauge
			}
